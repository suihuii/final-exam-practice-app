...
const outputPath = path.resolve("src/data/questions.json");
...
function scoreHeader(match: HeaderMatch): number {
  const candidateRows = collectCandidateRows(match);
  return candidateRows.length * 10 + match.optionColumns.length;
}

function findFormalRows(header: HeaderMatch): RawQuestionRow[] {
  const candidates = collectCandidateRows(header);
  if (candidates.length > 500) {
    return candidates.slice(candidates.length - 500);
  }
  return candidates;
}
